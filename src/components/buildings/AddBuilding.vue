          ...
          <v-col :cols=6>    
            <v-card class="elevation-12">
              ...
              <v-card-actions>
                ...
                <v-spacer ></v-spacer>
                <v-btn color="primary" @click.prevent="createBuilding($v)">{{ $t('create') }}</v-btn>
              </v-card-actions>
            </v-card>
          </v-col>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      createBuilding(validation) {
        ...
        const translateLocation = translateText(this.form.location).then(translated => formData.location = translated)

        Promise.all([translateName, translateDescription, translateLocation])
          .then(() => {
            this.$store.dispatch('createBuilding', formData)
