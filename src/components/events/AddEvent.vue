  ...
  <v-col :cols="8">
    <v-card class="elevation-12">
        ...
        <v-form>
          ...
          <v-row v-if="form.building_id">
            <v-col>
              ...
              ></v-time-picker>
              <span v-if="$v.form.date_from.$error" class="error-time">
            ...
            <v-col>
              ...
              ></v-time-picker>
              <span v-if="$v.form.date_to.$error" class="error-time">
      ...
      <v-card-actions>
        ...
        <v-spacer></v-spacer>
        <v-btn color="primary" @click.prevent="createEvent($v)">{{
          $t("create")
        }}</v-btn>
      </v-card-actions>
    </v-card>
...
export default {
  ...
  watch: {
    ...
    locale() {
      this.$store
        .dispatch("getAllBuildings", this.university.id)
        .then(buildings => {
          this.buildings = buildings;
        });
    }
  ...
  methods: {
    createEvent(validation) {
      ...
      Promise.all([translateName, translateDescription]).then(() => {
        this.$store
