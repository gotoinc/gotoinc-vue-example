      ...
      <v-col :cols=4 :key="event.id" v-for="event in events">
        ...
        >
          ...
          <v-card-text v-if="event.building && event.booking">
            ...
            <div class="my-4 subtitle-1">
              {{ event.building.name_locale }} - {{ event.building.location_locale }}
            </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      searchEvents() {
        this.events = []
        this.$store.dispatch('searchEvents', { s: this.search, university_id: this.university_id })
          .then((resp => {
      ...
      transformData(events) {
          return events.map(user => {
            ...
            if (tmp.building) {
              ...
              tmp.booking.date_from_locale = moment(tmp.booking.date_from).lang(this.locale).format('LT')
              tmp.booking.date_to_locale = moment(tmp.booking.date_to).lang(this.locale).format('LT')
            }
            return tmp
          })
      },
