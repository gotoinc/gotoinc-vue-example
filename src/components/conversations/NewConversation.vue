        ...
        <template v-for="(user, index) in users">
          <v-list-item :key="user.id">
            ...
            <v-tooltip bottom>
              ...
              <span
                >{{ $t("createConversationWith") + ` ${user.full_name}` }}
              </span>
            </v-tooltip>
          </v-list-item>
...
export default {
  ...
  methods: {
    getUsers(groupId) {
      this.$store.dispatch("getGroupUsers", groupId).then(resp => {
        let users = resp.data.data.map(user => user.attributes);
        console.log(this.alreadyCreatedConversationWithUsers);
        users = users.filter(
          user =>
            user.id != this.user.id &&
            !this.alreadyCreatedConversationWithUsers.includes(user.id)
        );
        this.users = users;
        this.allUsers = users;
      });
    ...
    searchUsers() {
      this.users = this.allUsers.filter(user =>
        user.full_name.toLowerCase().includes(this.inputModel.toLowerCase())
      );
    },
    createConversation(userId) {
      this.$store.dispatch("createConversation", userId).then(resp => {
    ...
    group: {
      handler(group) {
        ...
        if (this.alreadyCreatedConversationWithUsers.length) {
          ...
          this.$store.dispatch("getAllConversations").then(conversations => {
            this.alreadyCreatedConversationWithUsers = conversations.map(
              ({ partner }) => partner.id
            );
            this.getUsers(group.id);
          });
