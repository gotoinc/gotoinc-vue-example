...
export default {
  ...
  register({ commit, getters }, user) {
    return new Promise((resolve, reject) => {
      commit('auth_request');
      axios({
        url: `${env.api}/universities/${getters.university.id}/register_user`,
        data: user,
        method: 'POST',
      })
        .then(resp => {
  ...
  getGroups({ commit }, universityId) {
    axios({
      url: `${env.api}/universities/${universityId}/groups.json`,
      method: 'GET',
    })
      .then(resp => {
  ...
  registerUniversity(_, university) {
    return axios({
      url: `${env.api}/universities_non_auth`,
      data: university,
      method: 'POST',
    });
  },
  ...
  createGroup(_, { group, universityId }) {
    return axios({
      url: `${env.api}/universities/${universityId}/groups`,
      data: group,
      method: 'POST',
    });
  },
  ...
  searchEvents(_, { s, university_id }) {
    return axios({
      url: `${env.api}/events/search.json?s=${s}&university_id=${university_id}`,
      method: 'GET',
    });
  },
  ...
  willAttend(_, eventId) {
    return axios({
      url: `${env.api}/events/${eventId}/attend.json`,
      method: 'POST',
    });
  },

  getAllConversations({ state, commit }) {
    return axios({ url: `${env.api}/conversations.json`, method: 'GET' }).then(
      resp => {
        const conversations = resp.data.data.map(conv => {
          let tmp = conv.attributes;
          tmp.partner = tmp.users.find(
            ({ data }) => data.id != state.user.id
          ).data.attributes;
          tmp.chat_messages = tmp.chat_messages.map(
  ...
  getSingleConversation({ state, commit }, conversationId) {
    axios({
      url: `${env.api}/conversations/${conversationId}.json`,
      method: 'GET',
    }).then(resp => {
      let conversation = resp.data.data.attributes;
      conversation.partner = conversation.users.find(
        ({ data }) => data.id != state.user.id
      ).data.attributes;
      conversation.chat_messages = conversation.chat_messages.map(
        tmp => tmp.data.attributes
      );
  ...
  getGroupUsers(_, groupId) {
    return axios({
      url: `${env.api}/users.json?group_id=${groupId}`,
      method: 'GET',
    });
  },
  ...
  getAllBuildings({ state }, universityId) {
    return axios({
      url: `${env.api}/universities/${universityId}/buildings.json`,
      method: 'GET',
    }).then(resp => {
      const buildings = resp.data.data.map(building => {
        ...
        tmp.area =
          state.locale === 'en' ? Math.round(+tmp.area * 10.7639) : tmp.area;
        let startTime = tmp.available_time.mon[0];
        let endTime = tmp.available_time.mon[tmp.available_time.mon.length - 1];
        if (startTime && endTime) {
          tmp.availableTime =
            moment(`1.12.2020-${startTime}:00:00`)
              .lang(state.locale)
              .format('LT') +
            ' - ' +
            moment(`1.12.2020-${endTime}:00`)
              .lang(state.locale)
              .format('LT');
        }
        return tmp;
  ...
  deleteEvent(_, eventId) {
    return axios({
      url: `${env.api}/events/${eventId}.json`,
      method: 'DELETE',
    });
  },
  ...
  createBuilding({ getters }, data) {
    return axios({
      url: `${env.api}/universities/${getters.university.id}/buildings`,
      data,
      method: 'POST',
    });
  },
